package edu.pitt.sis.exp.colfusion.dal.orm;

// Generated Dec 3, 2014 8:12:54 PM by Hibernate Tools 3.4.0.CR1

/**
 * ColfusionPscSourceinfoTableId generated by hbm2java
 */
public class ColfusionPscSourceinfoTableId implements java.io.Serializable {

	private int sid;
	private String tableName;

	public ColfusionPscSourceinfoTableId() {
	}

	public ColfusionPscSourceinfoTableId(final int sid, final String tableName) {
		this.sid = sid;
		this.tableName = tableName;
	}

	public int getSid() {
		return this.sid;
	}

	public void setSid(final int sid) {
		this.sid = sid;
	}

	public String getTableName() {
		return this.tableName;
	}

	public void setTableName(final String tableName) {
		this.tableName = tableName;
	}

	@Override
	public boolean equals(final Object other) {
		if ((this == other)) {
			return true;
		}
		if ((other == null)) {
			return false;
		}
		if (!(other instanceof ColfusionPscSourceinfoTableId)) {
			return false;
		}
		ColfusionPscSourceinfoTableId castOther = (ColfusionPscSourceinfoTableId) other;

		return (this.getSid() == castOther.getSid())
				&& ((this.getTableName() == castOther.getTableName()) || (this
						.getTableName() != null
						&& castOther.getTableName() != null && this
						.getTableName().equals(castOther.getTableName())));
	}

	@Override
	public int hashCode() {
		int result = 17;

		result = 37 * result + this.getSid();
		result = 37 * result
				+ (getTableName() == null ? 0 : this.getTableName().hashCode());
		return result;
	}

}
