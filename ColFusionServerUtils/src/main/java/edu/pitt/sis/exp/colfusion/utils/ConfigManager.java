...
public final class ConfigManager {
	...
	private void loadPropertiesFromResoruceFile(final Properties prop, final String resourceName, final boolean isRequired) {
		final String propertiesLocation = ResourceUtils.getResourceAsFileLocation(this.getClass(), resourceName);
		final InputStream propertyFile = ResourceUtils.getResourceAsStream(this.getClass(), resourceName);
		if (propertyFile == null) {
			if (isRequired) {
				final String message = String.format("Was trying to load required properties from"
						+ "%s resource file, but got null.", resourceName);
				logger.error(message);
				throw new RuntimeException(message);
			}
			else {
				final String message = String.format("Didn't find property file %s in resource",
						CONFIG_FILE_NAME);
				logger.info(message);
			}
		}
	...
	void loadCustomPropertiesProvidedViaSystemProperty(final Properties prop)
			throws RuntimeException {
		final String propertyFilePath = getSystemProperty(PropertyKeys.CONFIG_FILE_NAME_SYSTEM_PROPERTY);
		if (propertyFilePath == null) {
			...
			try (InputStream propertiesFileStream = new FileInputStream(propertyFile)) {
				...
				final String message = String.format("The '%s' file provided in the '%s' system property doesn't exist. "
	...
	private void loadConfigFile(final Properties prop, final InputStream propertiesFileStream,
		...
		try {
			...
			final String message = String.format("Could not load properties.", propertiesFilename.toString());
	...
	public String getProperty(final IPropertyKeys propertyKey) {
		final String systemProperty = getSystemProperty(propertyKey);

		return systemProperty != null ? systemProperty: this.properties.getProperty(propertyKey.getKey());
	}
	...
	public String getProperty(final IPropertyKeys propertyKey, final String defaultValue) {
		...
		return systemProperty != null ? systemProperty : this.properties.getProperty(propertyKey.getKey(), defaultValue);
	}
