...
public class CancelPreview {
	 ...
	 * @throws SQLException 
	 */
	public GeneralResponseGen<String> cancelPreview(int sid, String tableName) throws SQLException {
		...
		if(tempTbExist(sid, tableName)) {
			rmTb(sid, tableName);
			cpTb(sid, "temp_" + tableName, tableName);
			rmTb(sid, "temp_" + tableName);
			msg = "Change has been cancelled!";
		} else {
			msg = "No change needs to be cancelled!";
		}
	...
	public void cpTb(int sid, String oriTb, String newTb) throws SQLException {
		String driver = "com.mysql.jdbc.Driver";
	    String dbURL = "jdbc:mysql://127.0.0.1:3306/colfusion_filetodb_" + sid; // Connect
	                                                                                // to
	                                                                                // Server
	    ...
	    String query = "CREATE TABLE " + newTb + " SELECT * FROM " + oriTb;
    	Statement ps = conn.createStatement();
